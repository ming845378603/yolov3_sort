...
def parse_args():
    ...
    # 置信度阈值设置
    parser.add_argument('--det_conf_thresh', default=0.3, type=float)# car1:0.3,car2:0.4
    # 最大允许丢失帧数
    parser.add_argument('--sort_max_age', default=5, type=int)
    #
    parser.add_argument('--sort_min_hit', default=3, type=int)
    # 输入文件
    parser.add_argument('--video',default="input/car1.mp4")
...
def track(frame, det_conf_thresh, colours):
    ...
    if len(result) > 0:
        ...
        for d in trackers:
            ...
            cv2.rectangle(im, (xmin,ymin), (xmax,ymax), (int(colours[id%32,0]), int(colours[id%32,1]), int(colours[id%32,2])), 3)
    ...
    cv2.putText(im, 'Current Obj Num: {}'.format(cur_obj_num), (20, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255,255,0), 2)
...
def trackOut():
    ...
    outputFile = args.video[6:-4]+'_Out.avi'
